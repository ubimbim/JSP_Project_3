	...
	public int insertOrderList(String shopid, String pnum, int order_no) {
		int result = 0, count = 0;
			...
			try {
				 ...
				 * 
				 * pstmt = con.prepareStatement(sql); pstmt.setString(1, shopid); rs =
				 * pstmt.executeQuery(); if(rs.next()) { count =
				 * Integer.parseInt(rs.getString(1).substring(8)) + 1; }else { count = 1; }
				 */
				...
				pstmt.setInt(4, count); // pstmt.setString(4, order_code+count);
				result = pstmt.executeUpdate();
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
